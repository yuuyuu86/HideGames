...
function MahjongGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  type State = { wall?: Tile[]; hands?: Record<string, Tile[]>; melds?: Record<string,Tile[][]>; openMelds?: Record<string,boolean[]>; turn?: string; discards?: Tile[]; discardedBy?: Record<string, Tile[]>; winner?: string; winType?: 'tsumo'|'ron'; winInfo?: MahjongWinInfo; scores?: Record<string, number>; riichi?: Record<string,boolean>; ippatsu?: Record<string,boolean>; rinshanId?: string; riichiPot?: number; lastDiscard?: { tile:Tile; owner:string }; calledMeld?: { kind:'pon'|'chi'|'kan'|'kakan' }; exhausted?: boolean; draw?: boolean; seatWinds?: Record<string,1|2|3|4>; roundWind?: 1|2|3|4; doraIndicators?: Tile[] }
  ...
  const winInfo=(hand:Tile[],type:'tsumo'|'ron',winningTile?:Tile)=>evaluateMahjongWin(hand,myMelds,{winType:type,riichi:state.riichi?.[playerId],ippatsu:state.ippatsu?.[playerId],rinshan:state.rinshanId===playerId,haitei:type==='tsumo'&&state.wall?.length===1&&state.rinshanId!==playerId,houtei:type==='ron'&&Boolean(state.exhausted),winningTile,seatWind:state.seatWinds?.[playerId],roundWind:state.roundWind,doraIndicators:state.doraIndicators,meldOpen:myOpenMelds})
  const start = () => { if(!host)return;const wall = buildWall(); const nextHands = Object.fromEntries(members.map((member, index) => [member.id, wall.slice(index * 13, index * 13 + 13)])); const remaining=wall.slice(members.length * 13); const dora=remaining.at(-1); const seatWinds=Object.fromEntries(members.map((member,index)=>[member.id,((index%4)+1) as 1|2|3|4])); setDeclaringRiichi(false);syncState({ wall: dora?remaining.slice(0,-1):remaining, hands: nextHands, melds:{}, openMelds:{}, turn: members[0]?.id, discards: [], discardedBy:{}, scores:Object.fromEntries(members.map(member=>[member.id,25000])), riichi: {}, ippatsu: {}, riichiPot:0, lastDiscard: undefined, draw: false, seatWinds, roundWind: 1, doraIndicators:dora?[dora]:[] }) }
