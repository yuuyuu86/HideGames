...
function TagGame({ paused, sharedState, moveTag, rematch, setMode, onFinished, playerId, members }: { paused: boolean; sharedState: unknown; moveTag: (position: { x: number; y: number }) => void; rematch: () => void; setMode: (mode: 'gems'|'escape'|'classic'|'infection') => void; onFinished: (result: 'win' | 'loss') => void; playerId: string; members: RoomMember[] }) {
  ...
  const host=members[0]?.id===playerId;const modeLabel={gems:'宝石回収',escape:'鍵で脱出',classic:'クラシック',infection:'感染鬼'}[mode]
  ...
  return <div className="board-game tag-game"><div className="game-top"><div><span className="tag">ACTION / {modeLabel.toUpperCase()}</span><h1>オンライン鬼ごっこ</h1></div><div className="turn-pill">鬼: <b>{hunterName}</b>{mode==='gems'&&<span className="gem-count"><FaGem /> {collected.length} / {gems.length}</span>}{mode==='escape'&&<span className="gem-count"><FaKey /> {keysCollected.length} / {keys.length}</span>}{(mode==='classic'||mode==='infection')&&<span>残り {remainingMoves} 手</span>}</div></div>{host&&<div className="go-actions">{(['gems','escape','classic','infection']as const).map(item=><button key={item} className={`secondary ${mode===item?'selected':''}`} disabled={paused||Boolean(winner)} onClick={()=>setMode(item)}>{({gems:'宝石回収',escape:'鍵で脱出',classic:'クラシック',infection:'感染鬼'}[item])}</button>)}</div>}<p className="game-hint">{mode==='gems'?'宝石を全て回収すると逃げる側の勝ちです。':mode==='escape'?'鍵を集め、出口へ到達すると逃げる側の勝ちです。':mode==='classic'?'鬼が全員を捕まえる前に、逃げる側は残り手数を耐えよう。':'感染鬼に捕まると鬼側へ加わります。残り手数まで逃げ切ろう。'} WASD または矢印キーで移動。ワープ床で反対側へ移動できます。</p>{winnerText && <div className="tag-result">{winnerText}<button className="secondary" onClick={rematch}>再戦する</button></div>}<div className="tag-map">{Array.from({length:96},(_,i)=>{const x=i%12,y=Math.floor(i/12);const self=pos.x===x&&pos.y===y;const other=Object.entries(positions).find(([id, point])=>id!==playerId&&point.x===x&&point.y===y);const gemIndex=gems.findIndex(gem=>gem.x===x&&gem.y===y);const keyIndex=keys.findIndex(key=>key.x===x&&key.y===y);const gem=mode==='gems'&&gemIndex>=0&&!collected.includes(gemIndex);const key=mode==='escape'&&keyIndex>=0&&!keysCollected.includes(keyIndex);const wall=(x===4&&y>1&&y<6)||(y===3&&x>6&&x<10);const warp=warps.some(point=>point.x===x&&point.y===y);const isExit=mode==='escape'&&exit.x===x&&exit.y===y;return <span key={i} className={wall?'wall':''}>{self?<FaPersonRunning />:other?<span className="remote-player" title={`${members.find(member=>member.id===other[0])?.name??'参加者'}${infected.includes(other[0])?'（感染鬼）':''}`}><FaUserSecret /></span>:gem?<FaGem />:key?<FaKey />:isExit?<FaDoorOpen />:warp?<FaMoon />:null}</span>})}</div></div>
}
...
function MahjongGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  type WinInfo = { yaku:string[]; han:number; points:number }
  type State = { wall?: Tile[]; hands?: Record<string, Tile[]>; melds?: Record<string,Tile[][]>; turn?: string; discards?: Tile[]; winner?: string; winType?: 'tsumo'|'ron'; winInfo?: WinInfo; riichi?: Record<string,boolean>; lastDiscard?: { tile:Tile; owner:string }; calledMeld?: { kind:'pon'|'chi'|'kan'|'ankan'|'kakan' }; exhausted?: boolean; draw?: boolean }
  ...
  const winInfo=(hand:Tile[],type:'tsumo'|'ron'):WinInfo|null=>{
    if(!winningHand(hand,myMelds.length))return null
    const all=[...hand,...myMelds.flat()];const yaku:string[]=[];let han=0;const add=(name:string,value:number)=>{yaku.push(name);han+=value}
    if(state.riichi?.[playerId])add('リーチ',1)
    if(type==='tsumo'&&!myMelds.length)add('門前清自摸和',1)
    if(all.every(tile=>tile.suit!=='z'&&tile.rank>1&&tile.rank<9))add('断么九',1)
    const honors=all.filter(tile=>tile.suit==='z');const numberSuits=new Set(all.filter(tile=>tile.suit!=='z').map(tile=>tile.suit));const open=myMelds.length>0
    if(honors.length===all.length)add('字一色',13)
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className={page === 'play' ? 'play-main' : 'main'}>
      ...
      {page === 'youtube' && <YoutubeScreenV2 url={youtubeUrl} setUrl={setYoutubeUrl} paused={room.paused} sharedState={room.gameState.youtube} syncState={(state)=>room.setGameState('youtube',state)} />}
      {page === 'friends' && <><FriendsScreen roomCode={room.roomCode} onInvite={room.sendChat} /><RecentPlayersPanel players={player.data.recentPlayers} blockedPlayers={player.data.blockedPlayers} roomCode={room.roomCode} onInvite={room.sendChat} onBlock={player.blockPlayer} onUnblock={player.unblockPlayer}/></>}
...
function YoutubeScreenV2({ url, setUrl, paused, sharedState, syncState }: { url: string; setUrl: (url: string) => void; paused: boolean; sharedState: unknown; syncState: (state: unknown) => void }) {
  ...
  const shared = sharedState as { url?: string; playing?: boolean; position?: number; startedAt?: number; pauseOnAway?: boolean } | undefined
...
function SettingsScreenV2({brightness,setBrightness,shortcut,setShortcut}:{brightness:number;setBrightness:(v:number)=>void;shortcut:string;setShortcut:(value:string)=>void}){
  type Preferences={bright:boolean;chat:boolean;notifications:boolean;chatNotifications:boolean;roomNotifications:boolean;fontScale:number;colorSafe:boolean;bgm:number;sfx:number;voice:number;youtube:number}
  const defaults:Preferences={bright:true,chat:true,notifications:true,chatNotifications:true,roomNotifications:true,fontScale:100,colorSafe:false,bgm:70,sfx:70,voice:80,youtube:80}
