...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className={page === 'play' ? 'play-main' : 'main'}>
      ...
      {page === 'friends' && <><FriendsScreen roomCode={room.roomCode} onInvite={room.sendChat} /><RecentPlayersPanel players={player.data.recentPlayers} roomCode={room.roomCode} onInvite={room.sendChat}/></>}
...
function RecentPlayersPanel({ players, roomCode, onInvite }: { players: PlayerData['recentPlayers']; roomCode: string; onInvite: (message: string) => void }) {
  if(!players.length)return null
  return <section className="page recent-players"><header className="section-head"><div><p className="eyebrow">RECENT PLAYERS</p><h2>最近遊んだ相手</h2></div></header><div className="panel">{players.slice(0,6).map(player=><div className="friend" key={player.id}><span className={`avatar ${player.color}`}>{initials(player.name)}</span><span><b>{player.name}</b><small>最近一緒に遊びました</small></span><button className="secondary" onClick={()=>onInvite(`${player.name} さんをルーム ${roomCode} に再招待しました`)}>再招待する</button></div>)}</div></section>
}
