...
function GoGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  const place = (row: number, col: number) => {
    if (paused || winner || turn !== (members[0]?.id === playerId ? 'b' : 'w') || board[row][col]) return
  ...
  const myColor=members[0]?.id===playerId?'b':'w'; const host=members[0]?.id===playerId
  const pass = () => { if(paused||winner||turn!==myColor)return;const nextPasses=passes+1;const stoneScore={b:board.flat().filter(stone=>stone==='b').length+captures.b,w:board.flat().filter(stone=>stone==='w').length+captures.w};const won=nextPasses>=2?(stoneScore.b===stoneScore.w?null:stoneScore.b>stoneScore.w?'b':'w'):null;const nextTurn=turn==='b'?'w':'b';setTurn(nextTurn);setPasses(nextPasses);setWinner(won);syncState({board,turn:nextTurn,captures,passes:nextPasses,winner:won}) }
  const reset=()=>{if(!host)return;const next=blank();setBoard(next);setTurn('b');setCaptures({b:0,w:0});setPasses(0);setWinner(null);syncState({board:next,turn:'b',captures:{b:0,w:0},passes:0,winner:null})}
  return <div className="board-game go-game"><div className="game-top"><div><span className="tag">BOARD / GO</span><h1>囲碁</h1></div><div className="turn-pill">{winner?`${winner==='b'?'黒':'白'}の勝ち`:`${turn===myColor?'あなた':'相手'}の番`}</div></div><div className="go-score"><span><i className="stone b"/>黒: {captures.b} 石を取る</span><span><i className="stone w"/>白: {captures.w} 石を取る</span></div><div className="go-board">{board.map((line,row)=>line.map((stone,col)=><button key={`${row}-${col}`} disabled={paused||Boolean(winner)||turn!==myColor} onClick={()=>place(row,col)} aria-label={`${row+1}行${col+1}列`}><span>{stone&&<i className={`stone ${stone}`}/>}</span></button>))}</div><div className="go-actions"><button className="secondary" disabled={paused||Boolean(winner)||turn!==myColor} onClick={pass}>パスする {passes ? `(${passes}/2)` : ''}</button>{winner&&<button className="primary" disabled={!host} onClick={reset}>新しい対局</button>}</div></div>
}
