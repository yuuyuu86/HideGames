...
function MahjongGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  const closedKanOptions=active===playerId&&!state.riichi?.[playerId]&&!state.winner&&!state.draw?[...new Set(visibleHand.map(tileKey))].filter(key=>visibleHand.filter(tile=>tileKey(tile)===key).length>=4):[]
