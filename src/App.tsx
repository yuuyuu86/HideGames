...
function MahjongGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  type State = { wall?: Tile[]; hands?: Record<string, Tile[]>; melds?: Record<string,Tile[][]>; openMelds?: Record<string,boolean[]>; turn?: string; discards?: Tile[]; discardedBy?: Record<string, Tile[]>; winner?: string; winType?: 'tsumo'|'ron'; winInfo?: MahjongWinInfo; scores?: Record<string, number>; riichi?: Record<string,boolean>; riichiPot?: number; lastDiscard?: { tile:Tile; owner:string }; calledMeld?: { kind:'pon'|'chi'|'kan'|'kakan' }; exhausted?: boolean; draw?: boolean; seatWinds?: Record<string,1|2|3|4>; roundWind?: 1|2|3|4; doraIndicators?: Tile[] }
  ...
  const pon=()=>{const tile=state.lastDiscard?.tile;if(paused||state.riichi?.[playerId]||state.winner||state.draw||!tile||state.lastDiscard?.owner===playerId||(mine.filter(item=>tileKey(item)===tileKey(tile)).length<2))return;let used=0;const nextHand=mine.filter(item=>{if(tileKey(item)===tileKey(tile)&&used<2){used++;return false}return true});syncState({...state,hands:{...hands,[playerId]:nextHand},melds:{...melds,[playerId]:[...myMelds,[tile,tile,tile]]},openMelds:{...(state.openMelds??{}),[playerId]:[...myOpenMelds,true]},turn:playerId,lastDiscard:undefined,calledMeld:{kind:'pon'},exhausted:false})}
  const kan=()=>{const tile=state.lastDiscard?.tile;if(paused||state.riichi?.[playerId]||state.winner||state.draw||!tile||state.lastDiscard?.owner===playerId||(mine.filter(item=>tileKey(item)===tileKey(tile)).length<3))return;let used=0;const nextHand=mine.filter(item=>{if(tileKey(item)===tileKey(tile)&&used<3){used++;return false}return true});syncState({...state,hands:{...hands,[playerId]:nextHand},melds:{...melds,[playerId]:[...myMelds,[tile,tile,tile,tile]]},openMelds:{...(state.openMelds??{}),[playerId]:[...myOpenMelds,true]},turn:playerId,lastDiscard:undefined,calledMeld:{kind:'kan'},exhausted:false})}
  ...
  const chi=(usedTiles:Tile[])=>{
    ...
    syncState({...state,hands:{...hands,[playerId]:remaining},melds:{...melds,[playerId]:[...myMelds,meld]},openMelds:{...(state.openMelds??{}),[playerId]:[...myOpenMelds,true]},turn:playerId,lastDiscard:undefined,calledMeld:{kind:'chi'},exhausted:false})
  }
