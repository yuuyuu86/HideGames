...
function RoomScreen({ start, selected, members, localMemberId, roomCode, onJoinRoom, onCreateRoom, sharedMemo, onMemo, sharedDrawing, onDrawing, onToggleReady, onSelectGame, awayHistory, tournamentState, onTournament, roomLocked, onSetPassword, onReport }: { start: () => void; selected: typeof games[number]; members: RoomMember[]; localMemberId: string; roomCode: string; onJoinRoom: (code: string, password?: string) => boolean; onCreateRoom: () => string; sharedMemo: string; onMemo: (text: string) => void; sharedDrawing: DrawLine[]; onDrawing: (lines: DrawLine[]) => void; onToggleReady: () => void; onSelectGame: (game: GameKey) => void; awayHistory: { id: string; name: string; away: boolean; at: number }[]; tournamentState: unknown; onTournament: (state: TournamentState) => void; roomLocked: boolean; onSetPassword: (password: string) => void; onReport: (targetId: string, reason: string) => void }) {
  ...
  return <div className="page room-page">
    <header className="room-header"><div><button className="back-link" onClick={()=>window.dispatchEvent(new Event('hidegames-open-home'))}><ChevronLeft size={17}/>ホーム</button><h1>{selected.title}</h1><p>ルームコード <b>{roomCode}</b></p></div><div className="room-invite-actions"><RoomQr roomCode={roomCode}/><button className="secondary" onClick={()=>{navigator.clipboard?.writeText(`hidegames://room/${roomCode}`);setCopied(true)}}><Copy size={16}/>{copied?'コピーしました':'招待リンクをコピー'}</button></div></header>
    ...
    <div className="room-layout">
      {local?.host && reconnecting.length > 0 && <section className="panel"><p className="eyebrow">HOST ACTION</p><h3>再接続を待つ参加者</h3>{reconnecting.map(member => <button key={member.id} className="text-button" onClick={() => { if (window.confirm(`${member.name} さんをルームから退出させますか？`)) window.dispatchEvent(new CustomEvent('hidegames-remove-member', { detail: member.id })) }}>{member.name} さんを退出させる</button>)}</section>}
      ...
      <section className="room-center"><div className="selected-game panel"><div className="selected-icon"><GameIcon /></div><div><span className="game-kind">{selected.kind}</span><h2>{selected.title}</h2><p>{selected.players} ・ {selected.time}　{selected.description}</p></div><button className="text-button" onClick={()=>setShowRules(value=>!value)} aria-expanded={showRules}><CircleHelp size={16}/>{showRules?'ルールを閉じる':'ルール'}</button></div>{showRules&&<section className="panel"><p className="eyebrow">HOW TO PLAY</p><h3>{selected.title}</h3><p>{selected.description}</p><small>必要人数: {selected.players}。開始前に参加者全員が準備OKになる必要があります。</small></section>}<div className="mode-card panel"><p className="eyebrow">GAME MODE</p><h3>宝石回収</h3><p>宝石を4つ集めて出口を目指そう。鬼はそれを阻止！</p><div className="room-game-picker">{games.filter(game=>game.ready).map(game=>{const Icon=game.Icon;return <button key={game.key} onClick={()=>onSelectGame(game.key as GameKey)} className={game.key===selected.key?'active':''}><Icon />{game.title}</button>})}</div></div><TournamentPanel members={members} game={selected} state={tournamentState} syncState={onTournament}/><button className="primary start" onClick={start} disabled={!canStart}><Play size={18} fill="currentColor"/>{canStart?'ゲームを開始':reconnecting.length?'再接続を待っています':members.length<minimumPlayers?`あと${minimumPlayers-members.length}人の参加を待っています`:'全員の準備を待っています'}</button></section>
...
function SettingsScreenV2({brightness,setBrightness,shortcut,setShortcut}:{brightness:number;setBrightness:(v:number)=>void;shortcut:string;setShortcut:(value:string)=>void}){
  ...
  const update=<K extends keyof Preferences>(key:K,value:Preferences[K])=>{
    if(key==='notifications'&&Boolean(value)&&'Notification' in window&&Notification.permission==='default'){
      void Notification.requestPermission().then(permission=>{setPrefs(current=>({...current,notifications:permission==='granted'}));setStatus(permission==='granted'?'通知を許可しました':'通知は許可されていません')})
      return
    }
