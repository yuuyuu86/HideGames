...
function MahjongGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  type WinInfo = { yaku:string[]; points:number }
  type State = { wall?: Tile[]; hands?: Record<string, Tile[]>; turn?: string; discards?: Tile[]; winner?: string; winType?: 'tsumo'|'ron'; winInfo?: WinInfo; riichi?: Record<string,boolean>; lastDiscard?: { tile:Tile; owner:string }; exhausted?: boolean; draw?: boolean }
  ...
  const winInfo=(hand:Tile[],type:'tsumo'|'ron'):WinInfo|null=>{if(!winningHand(hand))return null;const yaku:string[]=[];if(state.riichi?.[playerId])yaku.push('リーチ');if(type==='tsumo')yaku.push('門前清自摸和');if(hand.every(tile=>tile.suit!=='z'&&tile.rank>1&&tile.rank<9))yaku.push('断么九');const honors=hand.filter(tile=>tile.suit==='z');if(honors.length===hand.length)yaku.push('字一色');const honorCounts=honors.reduce<Record<number,number>>((counts,tile)=>({...counts,[tile.rank]:(counts[tile.rank]??0)+1}),{});Object.entries(honorCounts).filter(([,count])=>count>=3).forEach(([rank])=>yaku.push(`役牌 ${['東','南','西','北','白','發','中'][Number(rank)-1]}`));return yaku.length?{yaku,points:yaku.length*1000}:null}
  ...
  const tsumo=()=>{const hand=mine.length===13&&state.wall?.[0]?[...mine,state.wall[0]]:mine;const info=winInfo(hand,'tsumo');if(paused||active!==playerId||!info)return;syncState({...state,winner:playerId,winType:'tsumo',winInfo:info,lastDiscard:undefined})}
  const ron=()=>{const info=state.lastDiscard?winInfo([...mine,state.lastDiscard.tile],'ron'):null;if(paused||state.winner||state.draw||!state.lastDiscard||state.lastDiscard.owner===playerId||!info)return;syncState({...state,winner:playerId,winType:'ron',winInfo:info})}
