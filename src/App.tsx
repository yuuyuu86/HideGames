...
function MahjongGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  type State = { wall?: Tile[]; hands?: Record<string, Tile[]>; melds?: Record<string,Tile[][]>; openMelds?: Record<string,boolean[]>; turn?: string; discards?: Tile[]; discardedBy?: Record<string, Tile[]>; winner?: string; winType?: 'tsumo'|'ron'; winInfo?: MahjongWinInfo; scores?: Record<string, number>; riichi?: Record<string,boolean>; ippatsu?: Record<string,boolean>; rinshanId?: string; riichiPot?: number; lastDiscard?: { tile:Tile; owner:string }; calledMeld?: { kind:'pon'|'chi'|'kan'|'kakan' }; exhausted?: boolean; draw?: boolean; seatWinds?: Record<string,1|2|3|4>; roundWind?: 1|2|3|4; doraIndicators?: Tile[]; uraDoraIndicators?: Tile[] }
  const state = (sharedState as State | undefined) ?? {}; const hands = state.hands ?? {}; const melds=state.melds??{};const myMelds=melds[playerId]??[];const myOpenMelds=state.openMelds?.[playerId]??[];const concealed=13-myMelds.length*3;const active = state.turn ?? members[0]?.id; const mine = hands[playerId] ?? []
  ...
  const winInfo=(hand:Tile[],type:'tsumo'|'ron',winningTile?:Tile)=>evaluateMahjongWin(hand,myMelds,{winType:type,riichi:state.riichi?.[playerId],ippatsu:state.ippatsu?.[playerId],rinshan:state.rinshanId===playerId,haitei:type==='tsumo'&&state.wall?.length===1&&state.rinshanId!==playerId,houtei:type==='ron'&&Boolean(state.exhausted),tenhou:type==='tsumo'&&state.seatWinds?.[playerId]===1&&(state.discards?.length??0)===0,chiihou:type==='tsumo'&&state.seatWinds?.[playerId]!==1&&(state.discards?.length??0)===1&&!state.calledMeld,winningTile,seatWind:state.seatWinds?.[playerId],roundWind:state.roundWind,doraIndicators:state.doraIndicators,uraDoraIndicators:state.uraDoraIndicators,meldOpen:myOpenMelds})
  const start = () => { if(!host)return;const wall = buildWall(); const nextHands = Object.fromEntries(members.map((member, index) => [member.id, wall.slice(index * 13, index * 13 + 13)])); const remaining=wall.slice(members.length * 13); const dora=remaining.at(-1); const uraDora=remaining.at(-2); const seatWinds=Object.fromEntries(members.map((member,index)=>[member.id,((index%4)+1) as 1|2|3|4])); setDeclaringRiichi(false);syncState({ wall: dora?remaining.slice(0,-2):remaining, hands: nextHands, melds:{}, openMelds:{}, turn: members[0]?.id, discards: [], discardedBy:{}, scores:Object.fromEntries(members.map(member=>[member.id,25000])), riichi: {}, ippatsu: {}, riichiPot:0, lastDiscard: undefined, draw: false, seatWinds, roundWind: 1, doraIndicators:dora?[dora]:[], uraDoraIndicators:uraDora?[uraDora]:[] }) }
  const discard = (index: number) => { if (paused || active !== playerId || state.winner || state.draw || !state.wall?.length) return; const hand = mine.length === concealed ? [...mine, state.wall[0]] : mine; if(state.riichi?.[playerId]&&mine.length===concealed&&index!==hand.length-1)return; const discarded = hand[index]; if (!discarded) return; const nextHand=hand.filter((_, tileIndex) => tileIndex !== index); if(declaringRiichi&&(!tenpai(nextHand)||(state.scores?.[playerId]??25000)<1000))return;const nextHands = { ...hands, [playerId]: nextHand }; const nextTurn = members[(members.findIndex(member => member.id === playerId) + 1) % members.length]?.id ?? playerId; const remaining=mine.length===concealed?state.wall.slice(1):state.wall; const scores=declaringRiichi?{...Object.fromEntries(members.map(member=>[member.id,25000])),...(state.scores??{}),[playerId]:(state.scores?.[playerId]??25000)-1000}:state.scores; const ippatsu=declaringRiichi?{...(state.ippatsu??{}),[playerId]:true}:state.riichi?.[playerId]?{...(state.ippatsu??{}),[playerId]:false}:state.ippatsu; setDeclaringRiichi(false);syncState({ ...state, wall: remaining, hands: nextHands, scores, discards: [...(state.discards ?? []), discarded], discardedBy:{...(state.discardedBy??{}),[playerId]:[...(state.discardedBy?.[playerId]??[]),discarded]}, turn: nextTurn, riichi: declaringRiichi?{...(state.riichi??{}),[playerId]:true}:state.riichi, ippatsu, rinshanId:undefined, riichiPot:(state.riichiPot??0)+(declaringRiichi?1000:0), lastDiscard:{tile:discarded,owner:playerId}, exhausted:remaining.length===0 }) }
  const settledScores = (info: MahjongWinInfo, type: 'tsumo' | 'ron', loserId?: string) => {
    ...
    if (type === 'ron' && loserId) { const payment = winnerIsDealer ? info.payments.ron.dealer : info.payments.ron.nonDealer; scores[playerId] += payment + (state.riichiPot ?? 0); scores[loserId] = (scores[loserId] ?? 25000) - payment; return scores }
    ...
    scores[playerId] += state.riichiPot ?? 0
    return scores
  }
  ...
  const furiten = Boolean(state.lastDiscard && (state.discardedBy?.[playerId]??[]).some(tile=>tileKey(tile)===tileKey(state.lastDiscard!.tile)))
  const ron=()=>{const info=state.lastDiscard?winInfo([...mine,state.lastDiscard.tile],'ron',state.lastDiscard.tile):null;if(paused||state.winner||state.draw||furiten||!state.lastDiscard||state.lastDiscard.owner===playerId||!info)return;syncState({...state,winner:playerId,winType:'ron',winInfo:info,scores:settledScores(info,'ron',state.lastDiscard.owner)})}
  const pon=()=>{const tile=state.lastDiscard?.tile;if(paused||state.riichi?.[playerId]||state.winner||state.draw||!tile||state.lastDiscard?.owner===playerId||(mine.filter(item=>tileKey(item)===tileKey(tile)).length<2))return;let used=0;const nextHand=mine.filter(item=>{if(tileKey(item)===tileKey(tile)&&used<2){used++;return false}return true});syncState({...state,hands:{...hands,[playerId]:nextHand},melds:{...melds,[playerId]:[...myMelds,[tile,tile,tile]]},openMelds:{...(state.openMelds??{}),[playerId]:[...myOpenMelds,true]},ippatsu:{},rinshanId:undefined,turn:playerId,lastDiscard:undefined,calledMeld:{kind:'pon'},exhausted:false})}
  const kan=()=>{const tile=state.lastDiscard?.tile;if(paused||state.riichi?.[playerId]||state.winner||state.draw||!tile||state.lastDiscard?.owner===playerId||(mine.filter(item=>tileKey(item)===tileKey(tile)).length<3))return;let used=0;const nextHand=mine.filter(item=>{if(tileKey(item)===tileKey(tile)&&used<3){used++;return false}return true});syncState({...state,hands:{...hands,[playerId]:nextHand},melds:{...melds,[playerId]:[...myMelds,[tile,tile,tile,tile]]},openMelds:{...(state.openMelds??{}),[playerId]:[...myOpenMelds,true]},ippatsu:{},rinshanId:playerId,turn:playerId,lastDiscard:undefined,calledMeld:{kind:'kan'},exhausted:false})}
  const chiOptions=(()=>{const tile=state.lastDiscard?.tile;if(state.riichi?.[playerId]||!tile||tile.suit==='z')return [] as Tile[][];const ownerIndex=members.findIndex(member=>member.id===state.lastDiscard?.owner);if(active!==playerId||members[(ownerIndex+1)%members.length]?.id!==playerId)return [] as Tile[][];return [[tile.rank-2,tile.rank-1],[tile.rank-1,tile.rank+1],[tile.rank+1,tile.rank+2]].filter(ranks=>ranks.every(rank=>rank>=1&&rank<=9)).map(ranks=>ranks.map(rank=>mine.find(item=>item.suit===tile.suit&&item.rank===rank))).filter((items):items is Tile[]=>items.every(Boolean))})()
  const chi=(usedTiles:Tile[])=>{
    ...
    syncState({...state,hands:{...hands,[playerId]:remaining},melds:{...melds,[playerId]:[...myMelds,meld]},openMelds:{...(state.openMelds??{}),[playerId]:[...myOpenMelds,true]},ippatsu:{},rinshanId:undefined,turn:playerId,lastDiscard:undefined,calledMeld:{kind:'chi'},exhausted:false})
  }
  const visibleHand = active === playerId && mine.length === concealed && state.wall?.[0] ? [...mine, state.wall[0]] : mine
  const closedKanOptions=active===playerId&&mine.length===concealed&&!state.riichi?.[playerId]&&!state.winner&&!state.draw?[...new Set(visibleHand.map(tileKey))].filter(key=>visibleHand.filter(tile=>tileKey(tile)===key).length>=4):[]
  const closedKan=(key:string)=>{if(paused||active!==playerId||state.winner||state.draw||!closedKanOptions.includes(key))return;let used=0;const nextHand=visibleHand.filter(tile=>{if(tileKey(tile)===key&&used<4){used++;return false}return true});const meld=visibleHand.filter(tile=>tileKey(tile)===key).slice(0,4);const consumedDraw=mine.length===concealed&&Boolean(state.wall?.[0]);syncState({...state,wall:consumedDraw?state.wall?.slice(1):state.wall,hands:{...hands,[playerId]:nextHand},melds:{...melds,[playerId]:[...myMelds,meld]},openMelds:{...(state.openMelds??{}),[playerId]:[...myOpenMelds,false]},ippatsu:{},rinshanId:playerId,turn:playerId,lastDiscard:undefined,calledMeld:{kind:'ankan'},exhausted:consumedDraw&&state.wall?.length===1})}
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className={page === 'play' ? 'play-main' : 'main'}>
      ...
      {page === 'friends' && <><FriendsScreen roomCode={room.roomCode} onInvite={room.inviteFriend} /><RecentPlayersPanel players={player.data.recentPlayers} blockedPlayers={player.data.blockedPlayers} roomCode={room.roomCode} onInvite={room.inviteFriend} onBlock={player.blockPlayer} onUnblock={player.unblockPlayer}/></>}
...
function RecentPlayersPanel({ players, blockedPlayers, roomCode, onInvite, onBlock, onUnblock }: { players: PlayerData['recentPlayers']; blockedPlayers: PlayerData['blockedPlayers']; roomCode: string; onInvite: (targetId: string) => Promise<{ ok: boolean; message?: string }>; onBlock: (player: { id: string; name: string }) => void; onUnblock: (id: string) => void }) {
  const [notice, setNotice] = useState('')
  const [invitingId, setInvitingId] = useState('')
  const invite = async (player: PlayerData['recentPlayers'][number]) => {
    setInvitingId(player.id)
    try {
      const result = await onInvite(player.id)
      setNotice(result.ok ? `${player.name} さんへルーム ${roomCode} の招待を送りました` : result.message ?? '招待を送信できませんでした')
    } catch {
      setNotice('招待を送信できませんでした。通信状態を確認してください。')
    } finally {
      setInvitingId('')
    }
  }
  if(!players.length&&!blockedPlayers.length)return null
  return <section className="page recent-players"><header className="section-head"><div><p className="eyebrow">RECENT PLAYERS</p><h2>最近遊んだ相手</h2></div></header>{players.length>0&&<div className="panel"><p className="setting-hint">フレンドに追加済みの相手へ、今のルームの招待を送れます。</p>{players.slice(0,6).map(player=><div className="friend" key={player.id}><span className={`avatar ${player.color}`}>{initials(player.name)}</span><span><b>{player.name}</b><small>最近一緒に遊びました</small></span><button className="secondary" disabled={invitingId===player.id} onClick={()=>void invite(player)}>{invitingId===player.id ? '送信中...' : '再招待する'}</button><button className="text-button" onClick={()=>{if(window.confirm(`${player.name} さんをブロックしますか？ フレンド関係を解除し、今後の招待を拒否します。`))onBlock(player)}}>ブロック</button></div>)}{notice&&<p className="setting-status">{notice}</p>}</div>}{blockedPlayers.length>0&&<div className="panel blocked-players"><p className="eyebrow">BLOCKED</p>{blockedPlayers.map(player=><div className="friend" key={player.id}><span><b>{player.name}</b><small>招待を受け取らない設定です</small></span><button className="text-button" onClick={()=>onUnblock(player.id)}>ブロックを解除</button></div>)}</div>}</section>
}
function FriendsScreen({ roomCode, onInvite }: { roomCode: string; onInvite: (targetId: string) => Promise<{ ok: boolean; message?: string }> }) {
  ...
  useEffect(() => { if (!loggedIn) return; void loadRemoteFriends(); const refresh = () => void loadRemoteFriends(); window.addEventListener('hidegames-friends', refresh); const timer = window.setInterval(refresh, 20_000); return () => { window.removeEventListener('hidegames-friends', refresh); window.clearInterval(timer) } }, [loggedIn])
