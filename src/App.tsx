...
function MahjongGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  type State = { wall?: Tile[]; hands?: Record<string, Tile[]>; melds?: Record<string,Tile[][]>; turn?: string; discards?: Tile[]; winner?: string; winType?: 'tsumo'|'ron'; winInfo?: WinInfo; riichi?: Record<string,boolean>; lastDiscard?: { tile:Tile; owner:string }; exhausted?: boolean; draw?: boolean }
  ...
  const winInfo=(hand:Tile[],type:'tsumo'|'ron'):WinInfo|null=>{if(!winningHand(hand,myMelds.length))return null;const all=[...hand,...myMelds.flat()];const yaku:string[]=[];if(state.riichi?.[playerId])yaku.push('リーチ');if(type==='tsumo'&&!myMelds.length)yaku.push('門前清自摸和');if(all.every(tile=>tile.suit!=='z'&&tile.rank>1&&tile.rank<9))yaku.push('断么九');const honors=all.filter(tile=>tile.suit==='z');if(honors.length===all.length)yaku.push('字一色');const honorCounts=honors.reduce<Record<number,number>>((counts,tile)=>({...counts,[tile.rank]:(counts[tile.rank]??0)+1}),{});Object.entries(honorCounts).filter(([,count])=>count>=3).forEach(([rank])=>yaku.push(`役牌 ${['東','南','西','北','白','發','中'][Number(rank)-1]}`));return yaku.length?{yaku,points:yaku.length*1000}:null}
  const start = () => { if(!host)return;const wall = buildWall(); const nextHands = Object.fromEntries(members.map((member, index) => [member.id, wall.slice(index * 13, index * 13 + 13)])); setDeclaringRiichi(false);syncState({ wall: wall.slice(members.length * 13), hands: nextHands, melds:{}, turn: members[0]?.id, discards: [], riichi: {}, lastDiscard: undefined, draw: false }) }
  ...
  const tsumo=()=>{const hand=mine.length===concealed&&state.wall?.[0]?[...mine,state.wall[0]]:mine;const info=winInfo(hand,'tsumo');if(paused||active!==playerId||!info)return;syncState({...state,winner:playerId,winType:'tsumo',winInfo:info,lastDiscard:undefined})}
  ...
  const pon=()=>{const tile=state.lastDiscard?.tile;if(paused||state.winner||state.draw||!tile||state.lastDiscard?.owner===playerId||(mine.filter(item=>tileKey(item)===tileKey(tile)).length<2))return;let used=0;const nextHand=mine.filter(item=>{if(tileKey(item)===tileKey(tile)&&used<2){used++;return false}return true});syncState({...state,hands:{...hands,[playerId]:nextHand},melds:{...melds,[playerId]:[...myMelds,[tile,tile,tile]]},turn:playerId,lastDiscard:undefined,exhausted:false})}
