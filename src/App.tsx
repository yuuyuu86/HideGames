...
function Othello({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  const myColor = members[0]?.id === playerId ? 'b' : 'w'; const myName = members.find(member => member.id === playerId)?.name ?? 'あなた'; const opponentName = members.find(member => member.id !== playerId)?.name ?? '相手'; const myTurn = turn === myColor; const host = members[0]?.id === playerId
...
function Gomoku({ paused, sharedState, syncState }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void }) {
  ...
  const put=(r:number,c:number)=>{if(paused||winner||board[r][c])return;const b=board.map(row=>[...row]);b[r][c]=turn;setBoard(b);const ds=[[1,0],[0,1],[1,1],[1,-1]];const won=ds.some(([dr,dc])=>{let n=1;for(const k of[-1,1]){let y=r+dr*k,x=c+dc*k;while(y>=0&&y<size&&x>=0&&x<size&&b[y][x]===turn){n++;y+=dr*k;x+=dc*k}}return n>=5});const nextTurn=turn==='black'?'white':'black';if(won)setWinner(turn);else setTurn(nextTurn);syncState({board:b,turn:nextTurn,winner:won?turn:null})}
  return <div className="board-game"><div className="game-top"><div><span className="tag">ボードゲーム</span><h1>五目並べ</h1></div><div className="turn-pill">{winner ? `${winner==='black'?'あなた':'hana'} の勝ち！` : <><span className={`stone ${turn==='black'?'b':'w'}`} />{turn==='black'?'あなたの番です':'hana の番です'}</>}</div></div><div className="gomoku-board">{board.map((row,r)=>row.map((cell,c)=><button key={`${r}-${c}`} onClick={()=>put(r,c)} className="gomoku-cell">{cell&&<i className={`stone ${cell==='black'?'b':'w'}`} />}</button>))}</div>{winner&&<button className="secondary center" onClick={()=>{setBoard(Array.from({length:size},()=>Array<string|null>(size).fill(null)));setTurn('black');setWinner(null)}}>もう一度遊ぶ</button>}</div>
}

function ConnectFour({ paused, sharedState, syncState }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void }) {
  const rows = 6, cols = 7
  ...
  const reset = () => { const next = empty(); setBoard(next); setTurn('red'); setWinner(null); syncState({ board: next, turn: 'red', winner: null }) }
  return <div className="board-game"><div className="game-top"><div><span className="tag">ボードゲーム</span><h1>四目並べ</h1></div><div className="turn-pill">{winner ? `${winner === 'red' ? 'あなた' : 'hana'} の勝ち！` : <><span className={`connect-stone ${turn}`} />{turn === 'red' ? 'あなたの番です' : 'hana の番です'}</>}</div></div><div className="connect-board">{Array.from({ length: cols }, (_, column) => <button key={column} className="connect-column" onClick={() => drop(column)} aria-label={`${column + 1}列に置く`}>{board.map((line, row) => <span className={`connect-slot ${line[column] ?? ''}`} key={row} />)}</button>)}</div>{winner && <button className="secondary center" onClick={reset}>もう一度遊ぶ</button>}</div>
}

type UnoCard = { id: string; color: 'red' | 'blue' | 'yellow' | 'green' | 'wild'; value: string }
type ShogiPiece = `${'b'|'w'}${'K'|'R'|'B'|'G'|'S'|'N'|'L'|'P'}`
function ShogiGame({ paused, sharedState, syncState }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void }) {
  ...
  const [board,setBoard]=useState<(ShogiPiece|null)[][]>(initial);const [turn,setTurn]=useState<'b'|'w'>('b');const [selected,setSelected]=useState<[number,number]|null>(null);const [winner,setWinner]=useState<'b'|'w'|null>(null)
  useEffect(()=>{const state=sharedState as {board?:(ShogiPiece|null)[][];turn?:'b'|'w';winner?:'b'|'w'|null}|undefined;if(state?.board?.length===9&&state.turn){setBoard(state.board);setTurn(state.turn);setWinner(state.winner??null);setSelected(null)}},[sharedState])
  const valid=(from:[number,number],to:[number,number],piece=board[from[0]][from[1]])=>{if(!piece||from[0]===to[0]&&from[1]===to[1])return false;const target=board[to[0]][to[1]];if(target?.[0]===piece[0])return false;const dy=to[0]-from[0],dx=to[1]-from[1],ay=Math.abs(dy),ax=Math.abs(dx),forward=piece[0]==='b'?-1:1,kind=piece[1];const clear=()=>{const sy=Math.sign(dy),sx=Math.sign(dx);for(let y=from[0]+sy,x=from[1]+sx;y!==to[0]||x!==to[1];y+=sy,x+=sx)if(board[y][x])return false;return true};if(kind==='K')return ay<=1&&ax<=1;if(kind==='P')return dy===forward&&dx===0;if(kind==='L')return dx===0&&Math.sign(dy)===forward&&clear();if(kind==='N')return dy===2*forward&&ax===1;if(kind==='R')return (dx===0||dy===0)&&clear();if(kind==='B')return ay===ax&&clear();if(kind==='G')return (dy===forward&&ax<=1)||(dy===0&&ax===1)||(dy===-forward&&dx===0);if(kind==='S')return (dy===forward&&ax<=1)||(dy===-forward&&ax===1);return false}
  const move=(row:number,col:number)=>{if(paused||winner)return;const piece=board[row][col];if(!selected){if(piece?.[0]===turn)setSelected([row,col]);return}if(selected[0]===row&&selected[1]===col){setSelected(null);return}const moving=board[selected[0]][selected[1]];if(piece?.[0]===turn){setSelected([row,col]);return}if(!valid(selected,[row,col],moving))return;const next=board.map(line=>[...line]);const captured=next[row][col];next[row][col]=moving;next[selected[0]][selected[1]]=null;const won=captured?.[1]==='K'?turn:null;const nextTurn=turn==='b'?'w':'b';setBoard(next);setTurn(nextTurn);setWinner(won);setSelected(null);syncState({board:next,turn:nextTurn,winner:won})}
  const glyph:Record<ShogiPiece,string>={bK:'王',bR:'飛',bB:'角',bG:'金',bS:'銀',bN:'桂',bL:'香',bP:'歩',wK:'玉',wR:'飛',wB:'角',wG:'金',wS:'銀',wN:'桂',wL:'香',wP:'歩'}
  const reset=()=>{const next=initial();setBoard(next);setTurn('b');setWinner(null);setSelected(null);syncState({board:next,turn:'b',winner:null})}
  return <div className="board-game shogi-game"><div className="game-top"><div><span className="tag">BOARD / SHOGI</span><h1>将棋</h1></div><div className="turn-pill">{winner?`${winner==='b'?'先手':'後手'}の勝ち`:`${turn==='b'?'先手':'後手'}の番`}</div></div><p className="game-hint">駒を選び、移動先を選択します。持ち駒・成りは次の拡張対象です。</p><div className="shogi-board">{board.map((line,row)=>line.map((piece,col)=>{const active=selected?.[0]===row&&selected?.[1]===col;const destination=selected&&valid(selected,[row,col]);return <button key={`${row}-${col}`} disabled={paused||Boolean(winner)} className={`${active?'active':''} ${destination?'move-target':''}`} onClick={()=>move(row,col)}>{piece&&<span className={piece[0]==='w'?'opponent-piece':''}>{glyph[piece]}</span>}</button>}))}</div>{winner&&<button className="secondary center" onClick={reset}>新しい対局</button>}</div>
}
function DaifugoGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  type State={hands:Record<string,number[]>;turn:string;pile?:{rank:number;count:number;owner:string};passed:string[];ranking:string[]}
  ...
  const start=()=>{const deck=Array.from({length:52},(_,index)=>3+(index%13));const hands:Record<string,number[]>={};members.forEach((member,index)=>hands[member.id]=deck.filter((_,cardIndex)=>cardIndex%members.length===index).sort((a,b)=>a-b));syncState({hands,turn:members[0]?.id??playerId,passed:[],ranking:[]})}
  const canPlay=(rank:number)=>{if(!state)return false;const count=state.hands[playerId]?.filter(card=>card===rank).length??0;return count>=(state.pile?.count??1)&&(!state.pile||rank>state.pile.rank)}
  const play=(rank:number)=>{if(!state||paused||state.turn!==playerId||!canPlay(rank))return;const count=state.pile?.count??1;const hands={...state.hands,[playerId]:state.hands[playerId].filter((card,index)=>card!==rank||index>=state.hands[playerId].indexOf(rank)+count)};const ranking=hands[playerId].length===0?[...state.ranking,playerId]:state.ranking;syncState({...state,hands,pile:{rank,count,owner:playerId},turn:next(playerId,hands,ranking),passed:[],ranking})}
  const pass=()=>{if(!state||paused||state.turn!==playerId||!state.pile)return;const active=members.filter(member=>!state.ranking.includes(member.id)&&state.hands[member.id]?.length>0);const passed=[...state.passed,playerId];if(passed.length>=active.length-1){const lead=state.pile.owner;syncState({...state,pile:undefined,passed:[],turn:lead})}else syncState({...state,passed,turn:next(playerId,state.hands,state.ranking)})}
  ...
  const ranks=[...new Set(state.hands[playerId]??[])];const label=(rank:number)=>rank===14?'A':rank===15?'2':String(rank);const finished=state.ranking.length>=members.length-1
  return <div className="board-game daifugo-game"><div className="game-top"><div><span className="tag">CARD / DAIFUGO</span><h1>大富豪</h1></div><div className="turn-pill">{finished?'順位決定':`${members.find(member=>member.id===state.turn)?.name??'あなた'} の番`}</div></div><div className="uno-players">{members.map(member=><span key={member.id} className={member.id===state.turn?'active':''}>{state.ranking.includes(member.id)?`${state.ranking.indexOf(member.id)+1}位`:`${member.name} ${state.hands[member.id]?.length??0}枚`}</span>)}</div><section className="daifugo-pile panel"><p className="eyebrow">TABLE</p>{state.pile?<><b>{label(state.pile.rank)}</b><span>{state.pile.count}枚組 ・ {members.find(member=>member.id===state.pile?.owner)?.name}</span></>:<p>新しい場です。好きな数字を出せます。</p>}</section><div className="sevens-hand">{ranks.map(rank=><button key={rank} disabled={paused||state.turn!==playerId||!canPlay(rank)||finished} className={canPlay(rank)&&state.turn===playerId?'playable':''} onClick={()=>play(rank)}><small>{state.hands[playerId].filter(card=>card===rank).length}枚</small><b>{label(rank)}</b></button>)}</div><div className="go-actions"><button className="secondary" disabled={paused||state.turn!==playerId||!state.pile||finished} onClick={pass}>パスする</button>{finished&&<button className="primary" onClick={start}>もう一度遊ぶ</button>}</div></div>
}
...
function UnoGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  const play=(card:UnoCard)=>{if(!state||paused||state.turn!==playerId||!playable(card)||state.winner)return;let hands={...state.hands,[playerId]:state.hands[playerId].filter(item=>item.id!==card.id)};const direction=card.value==='reverse'?(state.direction*-1) as 1|-1:state.direction;const target=nextId(state.turn,direction);const drawCount=card.value==='+2'?2:card.value==='+4'?4:0;let drawIndex=state.drawIndex;if(drawCount){hands={...hands,[target]:[...(hands[target]??[]),...Array.from({length:drawCount},(_,index)=>deck[(drawIndex+index)%deck.length])]};drawIndex+=drawCount}const steps=card.value==='skip'||drawCount?2:1;syncState({...state,hands,pile:card,activeColor:card.color==='wild'?'red':card.color,direction,drawIndex,turn:nextId(state.turn,direction,steps),winner:hands[playerId].length===0?playerId:undefined})}
  ...
  const hand=state.hands[playerId]??[];const winner=state.winner?members.find(member=>member.id===state.winner):null
  return <div className="board-game uno-game"><div className="game-top"><div><span className="tag">CARD / UNO STYLE</span><h1>UNO風カードゲーム</h1></div><div className="turn-pill">{winner?`${winner.name} の勝ち`:`${members.find(member=>member.id===state.turn)?.name??'あなた'} の番`}</div></div><div className="uno-players">{members.map(member=><span key={member.id} className={member.id===state.turn?'active':''}>{member.name} <b>{state.hands[member.id]?.length??0}</b>枚</span>)}</div><section className="uno-table"><button className="uno-deck" disabled={paused||state.turn!==playerId||Boolean(winner)} onClick={draw}>DRAW</button><div className={`uno-card ${state.pile.color}`}><b>{cardLabel(state.pile)}</b><small>{state.activeColor==='wild'?'WILD':state.activeColor}</small></div></section><p className="game-hint">場札と同じ色、または同じ数字・記号のカードを出せます。WILDは赤として扱います。</p><div className="uno-hand">{hand.map(card=><button key={card.id} className={`uno-card ${card.color} ${playable(card)&&state.turn===playerId&&!winner?'playable':''}`} disabled={paused||state.turn!==playerId||!playable(card)||Boolean(winner)} onClick={()=>play(card)}><b>{cardLabel(card)}</b><small>{card.color==='wild'?'WILD':card.color}</small></button>)}</div>{winner&&<button className="primary center" onClick={begin}>もう一度遊ぶ</button>}</div>
}
type ChessPiece = `${'w' | 'b'}${'k' | 'q' | 'r' | 'b' | 'n' | 'p'}`
function GoGame({ paused, sharedState, syncState }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void }) {
  type Stone = 'b' | 'w' | null
  const size = 9
  ...
  useEffect(() => { const state = sharedState as { board?: Stone[][]; turn?: 'b'|'w'; captures?: {b:number;w:number}; passes?: number; winner?: 'b'|'w'|null } | undefined; if (state?.board?.length === size && state.turn) { setBoard(state.board);setTurn(state.turn);setCaptures(state.captures ?? {b:0,w:0});setPasses(state.passes ?? 0);setWinner(state.winner ?? null) } }, [sharedState])
  ...
  const place = (row: number, col: number) => {
    if (paused || winner || board[row][col]) return
    ...
    const nextCaptures={...captures,[turn]:captures[turn]+taken};const nextTurn=other;setBoard(next);setTurn(nextTurn);setCaptures(nextCaptures);setPasses(0);syncState({board:next,turn:nextTurn,captures:nextCaptures,passes:0,winner:null})
  }
  const pass = () => { if(paused||winner)return;const nextPasses=passes+1;const stoneScore={b:board.flat().filter(stone=>stone==='b').length+captures.b,w:board.flat().filter(stone=>stone==='w').length+captures.w};const won=nextPasses>=2?(stoneScore.b===stoneScore.w?null:stoneScore.b>stoneScore.w?'b':'w'):null;const nextTurn=turn==='b'?'w':'b';setTurn(nextTurn);setPasses(nextPasses);setWinner(won);syncState({board,turn:nextTurn,captures,passes:nextPasses,winner:won}) }
  const reset=()=>{const next=blank();setBoard(next);setTurn('b');setCaptures({b:0,w:0});setPasses(0);setWinner(null);syncState({board:next,turn:'b',captures:{b:0,w:0},passes:0,winner:null})}
  return <div className="board-game go-game"><div className="game-top"><div><span className="tag">BOARD / GO</span><h1>囲碁</h1></div><div className="turn-pill">{winner?`${winner==='b'?'黒':'白'}の勝ち`:`${turn==='b'?'黒':'白'}の番`}</div></div><div className="go-score"><span><i className="stone b"/>黒: {captures.b} 石を取る</span><span><i className="stone w"/>白: {captures.w} 石を取る</span></div><div className="go-board">{board.map((line,row)=>line.map((stone,col)=><button key={`${row}-${col}`} disabled={paused||Boolean(winner)} onClick={()=>place(row,col)} aria-label={`${row+1}行${col+1}列`}><span>{stone&&<i className={`stone ${stone}`}/>}</span></button>))}</div><div className="go-actions"><button className="secondary" disabled={paused||Boolean(winner)} onClick={pass}>パスする {passes ? `(${passes}/2)` : ''}</button>{winner&&<button className="primary" onClick={reset}>新しい対局</button>}</div></div>
}
function ChessGame({ paused, sharedState, syncState }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void }) {
  ...
  const reset = () => { const next=initial();setBoard(next);setTurn('w');setWinner(null);setSelected(null);syncState({board:next,turn:'w',winner:null}) }
  return <div className="board-game chess-game"><div className="game-top"><div><span className="tag">BOARD / CHESS</span><h1>チェス</h1></div><div className="turn-pill">{winner ? `${winner==='w'?'白':'黒'}の勝ち` : `${turn==='w'?'白':'黒'}の番`}</div></div><p className="game-hint">駒を選び、移動先を選択します。ポーンは最終列で自動的にクイーンへ昇格します。</p><div className="chess-board">{board.map((line,row)=>line.map((piece,col)=>{const active=selected?.[0]===row&&selected?.[1]===col;const destination=selected&&valid(selected,[row,col]);return <button key={`${row}-${col}`} disabled={paused||Boolean(winner)} onClick={()=>move(row,col)} className={`${(row+col)%2?'dark':'light'} ${active?'active':''} ${destination?'move-target':''}`} aria-label={`${row+1}行${col+1}列`}>{piece&&<span className={piece[0]==='w'?'white-piece':'black-piece'}>{glyph[piece]}</span>}</button>}))}</div>{winner&&<button className="secondary center" onClick={reset}>新しい対局</button>}</div>
}
...
const buildWall = (): Tile[] => ['m', 'p', 's'].flatMap(suit => Array.from({ length: 9 }, (_, rank) => Array.from({ length: 4 }, () => ({ suit: suit as Tile['suit'], rank: rank + 1 }))).flat()).concat(Array.from({ length: 7 }, (_, rank) => Array.from({ length: 4 }, () => ({ suit: 'z' as const, rank: rank + 1 }))).flat()).sort((a, b) => tileKey(a).localeCompare(tileKey(b)))
const winningHand = (hand: Tile[]) => {
  ...
  for (const key of counts.keys()) { if (!take(key, 2)) continue; const win = groups(12); put(key, 2); if (win) return true }
  return false
}
function MahjongGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  type State = { wall?: Tile[]; hands?: Record<string, Tile[]>; turn?: string; discards?: Tile[]; winner?: string }
  const state = (sharedState as State | undefined) ?? {}; const hands = state.hands ?? {}; const active = state.turn ?? members[0]?.id; const mine = hands[playerId] ?? []
  const start = () => { const wall = buildWall(); const nextHands = Object.fromEntries(members.map((member, index) => [member.id, wall.slice(index * 13, index * 13 + 13)])); syncState({ wall: wall.slice(members.length * 13), hands: nextHands, turn: members[0]?.id, discards: [] }) }
  const discard = (index: number) => { if (paused || active !== playerId || state.winner || !state.wall?.length) return; const hand = mine.length === 13 ? [...mine, state.wall[0]] : mine; const discarded = hand[index]; if (!discarded) return; const nextHands = { ...hands, [playerId]: hand.filter((_, tileIndex) => tileIndex !== index) }; const nextTurn = members[(members.findIndex(member => member.id === playerId) + 1) % members.length]?.id ?? playerId; const remaining = state.wall.slice(1); syncState({ ...state, wall: remaining, hands: nextHands, discards: [...(state.discards ?? []), discarded], turn: nextTurn, winner: winningHand(hand) ? playerId : undefined }) }
  const visibleHand = active === playerId && mine.length === 13 && state.wall?.[0] ? [...mine, state.wall[0]] : mine
  return <div className="board-game mahjong-game"><div className="game-top"><div><span className="tag">TABLE / RIICHI LITE</span><h1>麻雀</h1></div><div className="turn-pill">{state.winner ? `${members.find(m => m.id === state.winner)?.name} の和了` : active ? `${members.find(m => m.id === active)?.name ?? 'プレイヤー'} の番` : '卓を開始'}</div></div>{!state.wall ? <section className="mahjong-start panel"><h2>4面子1雀頭を作ろう</h2><p>山から1枚引き、不要な牌を1枚捨てます。順子・刻子と雀頭で14枚をそろえると和了です。</p><button className="primary" disabled={paused} onClick={start}>対局を開始</button></section> : <><section className="mahjong-table panel"><div><b>残り {state.wall.length} 枚</b><span>捨て牌: {(state.discards ?? []).slice(-12).map(tileLabel).join(' ') || 'なし'}</span></div><p>{active === playerId ? '牌を1枚選んで捨ててください。' : '相手の手番を待っています。'}</p></section><section className="mahjong-hand">{visibleHand.map((tile, index) => <button key={`${tileKey(tile)}-${index}`} className={`mahjong-tile ${tile.suit}`} disabled={paused || active !== playerId || Boolean(state.winner)} onClick={() => discard(index)}>{tileLabel(tile)}</button>)}</section></>}</div>
}
...
function PuzzleGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  const move = (dx: number, rotation = 0) => { if (paused || turn !== playerId || state.finished) return; const next = { ...active, x: active.x + dx, rot: active.rot + rotation }; if (canPlace(next)) syncState({ ...state, board, active: next, turn, score }) }
  const drop = () => { if (paused || turn !== playerId || state.finished) return; let landed = active; while (canPlace({ ...landed, y: landed.y + 1 })) landed = { ...landed, y: landed.y + 1 }; const next = board.map(row => [...row]); shapeCells(landed).forEach(cell => { if (cell.y >= 0) next[cell.y][cell.x] = (cell.value % 4 + 1) as Block }); const seen = new Set<string>(); const remove = new Set<string>(); const visit = (x:number,y:number,value:Block,group:string[]) => { const key=`${x}:${y}`; if(seen.has(key)||x<0||x>=6||y<0||y>=12||next[y][x]!==value)return;seen.add(key);group.push(key);visit(x+1,y,value,group);visit(x-1,y,value,group);visit(x,y+1,value,group);visit(x,y-1,value,group) }; next.forEach((row,y)=>row.forEach((value,x)=>{if(!value||seen.has(`${x}:${y}`))return;const group:string[]=[];visit(x,y,value,group);if(group.length>=4)group.forEach(key=>remove.add(key))})); remove.forEach(key=>{const [x,y]=key.split(':').map(Number);next[y][x]=0}); for(let x=0;x<6;x++){const column=next.map(row=>row[x]).filter(Boolean);for(let y=11;y>=0;y--)next[y][x]=(column.pop()??0) as Block} const nextTurn=members[(members.findIndex(member=>member.id===turn)+1)%members.length]?.id??playerId;const nextScore={...score,[playerId]:(score[playerId]??0)+remove.size};const nextActive={x:2,y:0,shape:(active.shape+1)%tetrominoes.length,rot:0};syncState({board:next,active:nextActive,turn:nextTurn,score:nextScore,finished:!canPlace(nextActive)}) }
  const activeCells = new Map(shapeCells(active).map(cell => [`${cell.x}:${cell.y}`, ((cell.value % 4) + 1) as Block]))
  return <div className="board-game falling-game"><div className="game-top"><div><span className="tag">COLOUR / CHAIN PUZZLE</span><h1>落ちものパズル</h1></div><div className="turn-pill">{state.finished ? 'ゲーム終了' : `${members.find(m => m.id === turn)?.name ?? 'あなた'} の番`}</div></div><section className="falling-layout"><div className="falling-board puzzle-board">{board.flatMap((row,y)=>row.map((value,x)=><i key={`${x}-${y}`} className={`block orb c${activeCells.get(`${x}:${y}`) ?? value}`}/>))}</div><section className="falling-side panel"><h2>4つつなげて消す</h2><p>同じ色を4つ以上隣接させると消えます。消した個数が得点になります。</p><div className="falling-controls"><button disabled={paused || turn !== playerId} onClick={()=>move(-1)}>左</button><button disabled={paused || turn !== playerId} onClick={()=>move(0,1)}>回転</button><button disabled={paused || turn !== playerId} onClick={()=>move(1)}>右</button><button className="primary" disabled={paused || turn !== playerId} onClick={drop}>落とす</button></div><div className="falling-score">{members.map(member=><div key={member.id}><span>{member.name}</span><b>{score[member.id]??0} POINT</b></div>)}</div></section></section></div>
}
...
function RoundGame({ game, paused, sharedState, syncState, members, playerId }: { game: typeof games[number]; paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  if (game.key === 'shogi') return <ShogiGame paused={paused} sharedState={sharedState} syncState={syncState} />
...
function App() {
  ...
  useEffect(()=>{const shortcut=(e:KeyboardEvent)=>{if(window.hideGamesDesktop)return;if(e.ctrlKey&&e.shiftKey&&e.key.toLowerCase()==='h'){e.preventDefault();room.setPaused(true);setShowShortcut(true)}};window.addEventListener('keydown',shortcut);return()=>window.removeEventListener('keydown',shortcut)},[room])
  useEffect(()=>{if(['tag','othello','gomoku','connect4'].includes(room.game)) setSelected(room.game as GameKey)},[room.game])
  ...
  return <div className="app-shell">
    ...
    <main className={page === 'play' ? 'play-main' : 'main'}>
      ...
      {page === 'youtube' && <YoutubeScreenV2 url={youtubeUrl} setUrl={setYoutubeUrl} sharedState={room.gameState.youtube} syncState={(state)=>room.setGameState('youtube',state)} />}
      {page === 'friends' && <FriendsScreen roomCode={room.roomCode} onInvite={room.sendChat} />}
      {page === 'profile' && <><AuthPanel defaultName={player.data.displayName}/><ProfileScreen data={player.data} onName={player.updateProfile} /></>}
...
function HomeScreen({ setPage, start, favourites }: { setPage:(p:Page)=>void; start:(g:GameKey)=>void; favourites:string[] }) { const favouriteGames=games.filter(game=>favourites.includes(game.key)).slice(0,3); return <div className="page home-page"><header className="topbar"><div><p className="eyebrow">FRIENDS, GAMES, ONE PLACE</p><h1>友達と、すぐ遊ぼう。</h1><p>ルームを作って集まる。急な離席も、みんなで待てる。</p></div><button className="icon-button"><Bell size={19}/></button></header><section className="hero-actions"><button className="action-card create" onClick={()=>setPage('room')}><Plus/><span><b>ルームを作る</b><small>ゲームを選んで友達を招待</small></span><span className="action-arrow">→</span></button><button className="action-card join" onClick={()=>setPage('room')}><span className="join-icon"><Users /></span><span><b>コードで入る</b><small>ルームコードを入力して参加</small></span><span className="action-arrow">→</span></button></section><section className="daily-card panel"><div><p className="eyebrow">DAILY CHALLENGE</p><h2>今日の協力ミッション</h2><p>協力マインスイーパーを地雷に当たらずクリアしよう。</p></div><button className="primary" onClick={()=>start('mines')}>挑戦する</button></section>{favouriteGames.length>0&&<><section className="section-head compact"><div><p className="eyebrow">FAVOURITES</p><h2>お気に入り</h2></div></section><div className="favourite-row">{favouriteGames.map(game=>{const Icon=game.Icon;return <button key={game.key} onClick={()=>start(game.key)}><Icon/><span>{game.title}</span></button>})}</div></>}<section className="section-head compact"><div><p className="eyebrow">CONTINUE PLAYING</p><h2>最近のルーム</h2></div><button className="text-button" onClick={()=>setPage('room')}>すべて見る →</button></section><div className="room-list">{[{title:'オンライン鬼ごっこ',detail:'4/6人 ・ 作成者: yuta',key:'tag'},{title:'オセロ対戦部屋',detail:'2/2人 ・ 再戦待ち',key:'othello'},{title:'五目並べ練習',detail:'1/2人 ・ 作成者: hana',key:'gomoku'}].map((r,i)=>{const Icon=games.find(g=>g.key===r.key)?.Icon;return <button className="room-row" key={r.title} onClick={()=>start(r.key as GameKey)}><span className={`game-orb orb-${i}`}>{Icon&&<Icon />}</span><span><b>{r.title}</b><small>{r.detail}</small></span><span className="join-small">参加</span></button>})}</div></div> }
function GamesScreen({ start, favourites, onFavourite }: { start:(g:GameKey)=>void; favourites:string[]; onFavourite:(game:string)=>void }) { const [filter,setFilter]=useState('すべて'); const [query,setQuery]=useState(''); const shown=(filter==='すべて'?games:games.filter(g=>g.kind===filter)).filter(game=>`${game.title} ${game.kind}`.includes(query.trim()));return <div className="page"><header className="page-title"><div><p className="eyebrow">GAME LIBRARY</p><h1>ゲームを選ぶ</h1><p>人数や気分に合わせて、すぐルームを作れます。</p></div><label className="search"><Search size={17}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="ゲームを検索" /></label></header><div className="filters">{['すべて','ボード','カード','アクション','協力・推理','パーティー'].map(f=><button key={f} className={filter===f?'selected':''} onClick={()=>setFilter(f)}>{f}</button>)}</div><div className="game-grid">{shown.map(g=>{const Icon=g.Icon;const favourite=favourites.includes(g.key);return <article className={`game-card ${g.ready?'':'coming'}`} key={g.key}><div className="game-visual"><Icon /><button className={`favourite-button ${favourite?'saved':''}`} aria-label={`${g.title}をお気に入りにする`} onClick={()=>onFavourite(g.key)}><FaRegStar/></button><em>{g.ready?'今すぐ遊べる':'準備中'}</em></div><div className="game-copy"><div><span className="game-kind">{g.kind}</span><h2>{g.title}</h2></div><p>{g.description}</p><small><Users size={14}/>{g.players}<span/><Circle size={11}/> {g.time}</small></div><button className={g.ready?'primary':'secondary'} disabled={!g.ready} onClick={()=>g.ready&&start(g.key as GameKey)}>{g.ready?'このゲームで遊ぶ':'近日追加'}</button></article>})}</div>{shown.length===0&&<p className="empty-copy">該当するゲームが見つかりません。別の言葉で検索してください。</p>}</div> }
...
function RoomScreen({ start, selected, members, localMemberId, roomCode, onJoinRoom, onCreateRoom, sharedMemo, onMemo, sharedDrawing, onDrawing, onToggleReady, onSelectGame, awayHistory, tournamentState, onTournament, roomLocked, onSetPassword, onReport }: { start: () => void; selected: typeof games[number]; members: RoomMember[]; localMemberId: string; roomCode: string; onJoinRoom: (code: string, password?: string) => boolean; onCreateRoom: () => string; sharedMemo: string; onMemo: (text: string) => void; sharedDrawing: DrawLine[]; onDrawing: (lines: DrawLine[]) => void; onToggleReady: () => void; onSelectGame: (game: GameKey) => void; awayHistory: { id: string; name: string; away: boolean; at: number }[]; tournamentState: unknown; onTournament: (state: TournamentState) => void; roomLocked: boolean; onSetPassword: (password: string) => void; onReport: (targetId: string, reason: string) => void }) {
  ...
  const canStart = members.length > 0 && members.every(member => member.ready && member.connected !== false)
  return <div className="page room-page">
    <header className="room-header"><div><button className="back-link"><ChevronLeft size={17}/>ホーム</button><h1>{selected.title}</h1><p>ルームコード <b>{roomCode}</b></p></div><div className="room-invite-actions"><RoomQr roomCode={roomCode}/><button className="secondary" onClick={()=>{navigator.clipboard?.writeText(`hidegames://room/${roomCode}`);setCopied(true)}}><Copy size={16}/>{copied?'コピーしました':'招待リンクをコピー'}</button></div></header>
    <section className="room-access panel"><div><b>別のルームに参加</b><small>6文字のルームコードと、必要な場合はパスワードを入力</small></div><form onSubmit={event=>{event.preventDefault();if(onJoinRoom(inputCode,inputPassword)){setInputCode('');setInputPassword('');setRoomError('')}else setRoomError('英数字6文字のコードを入力してください')}}><input aria-label="ルームコード" value={inputCode} maxLength={6} onChange={event=>setInputCode(event.target.value.toUpperCase())} placeholder="例: A7K9P2"/><input aria-label="ルームパスワード" value={inputPassword} type="password" onChange={event=>setInputPassword(event.target.value)} placeholder="パスワード（任意）"/><button className="secondary">参加する</button><button type="button" className="primary" onClick={()=>{onCreateRoom();setRoomError('')}}>新しいルームを作る</button></form>{members.find(member=>member.id===localMemberId)?.host&&<div className="room-password"><label>このルームのパスワード<input type="password" value={roomPassword} onChange={event=>setRoomPassword(event.target.value)} placeholder={roomLocked?'設定済み':'空欄で公開ルーム'}/></label><button className="text-button" onClick={()=>onSetPassword(roomPassword)}>{roomLocked?'パスワードを更新':'パスワードを設定'}</button></div>}{roomError&&<p role="alert">{roomError}</p>}</section>
    <div className="room-layout">
      <section className="room-members panel"><div className="panel-title"><h2>参加メンバー <small>{members.length} / 8</small></h2><span className={`live-dot ${reconnecting.length ? 'reconnecting' : ''}`}>{reconnecting.length ? `再接続中 ${reconnecting.length}人` : '全員接続中'}</span></div>{members.map(member=><div className={`member ${member.connected === false ? 'reconnecting' : ''}`} key={member.id}><span className={`avatar ${member.color}`}>{initials(member.name)}</span><span className="member-name">{member.name}{member.host&&<Crown size={14}/>}</span><span className={`ready ${member.ready?'yes':''}`}>{member.connected === false ? <><Radio size={13}/>再接続中</> : member.away ? '一時離席中' : <>{member.ready?<CheckCircle2 size={14}/>:<Circle size={14}/>} {member.ready?'準備OK':'準備中'}</>}</span>{member.id!==localMemberId&&<button className="member-report" onClick={()=>{if(window.confirm(`${member.name} さんを通報しますか？`))onReport(member.id,'ルーム内通報')}}>通報</button>}</div>)}<button className="secondary ready-toggle" onClick={onToggleReady}>{local?.ready?<Circle size={16}/>:' '} {local?.ready?'準備を解除':'準備OKにする'}</button><button className="invite"><Plus size={16}/>フレンドを招待</button>{awayHistory.length>0&&<div className="away-history"><b>離席履歴</b>{awayHistory.slice(-3).reverse().map(item=><small key={item.id}>{item.name} さんが{item.away?'離席':'復帰'}しました</small>)}</div>}</section>
      <section className="room-center"><div className="selected-game panel"><div className="selected-icon"><GameIcon /></div><div><span className="game-kind">{selected.kind}</span><h2>{selected.title}</h2><p>{selected.players} ・ {selected.time}　{selected.description}</p></div><button className="text-button"><CircleHelp size={16}/>ルール</button></div><div className="mode-card panel"><p className="eyebrow">GAME MODE</p><h3>宝石回収</h3><p>宝石を4つ集めて出口を目指そう。鬼はそれを阻止！</p><div className="room-game-picker">{games.filter(game=>game.ready).map(game=>{const Icon=game.Icon;return <button key={game.key} onClick={()=>onSelectGame(game.key as GameKey)} className={game.key===selected.key?'active':''}><Icon />{game.title}</button>})}</div></div><TournamentPanel members={members} game={selected} state={tournamentState} syncState={onTournament}/><button className="primary start" onClick={start} disabled={!canStart}><Play size={18} fill="currentColor"/>{canStart?'ゲームを開始':reconnecting.length?'再接続を待っています':'全員の準備を待っています'}</button></section>
      <section className="room-side panel"><div className="tabs"><button className={sideTab==='memo'?'active':''} onClick={()=>setSideTab('memo')}>共有メモ</button><button className={sideTab==='drawing'?'active':''} onClick={()=>setSideTab('drawing')}>お絵描き</button></div>{sideTab==='memo'?<div className="memo-box"><label htmlFor="shared-memo">ルームのみんなで編集できます</label><textarea id="shared-memo" value={sharedMemo} onChange={event=>onMemo(event.target.value)} placeholder="作戦、ルール、手がかりをメモ…" /></div>:<DrawingBoard lines={sharedDrawing} onChange={onDrawing}/>}<button className="youtube-mini"><Video size={16}/>動画URLを貼り付ける</button></section>
    </div>
  </div>
}
function YoutubeScreenV2({ url, setUrl, sharedState, syncState }: { url: string; setUrl: (url: string) => void; sharedState: unknown; syncState: (state: unknown) => void }) {
...
function AuthPanel({ defaultName }: { defaultName: string }) {
  ...
  const submit=async(event:React.FormEvent)=>{event.preventDefault();setStatus('認証中…');try{const localHost=['localhost','127.0.0.1'].includes(window.location.hostname);const base=import.meta.env.VITE_SOCKET_URL||(localHost?`http://${window.location.hostname}:3001`:window.location.origin);const response=await fetch(`${base}/auth/${mode}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password,displayName:name})});const body=await response.json() as {token?:string;user?:{displayName:string};error?:string};if(!response.ok)throw new Error(body.error??'認証に失敗しました');localStorage.setItem('hidegames.auth-token',body.token??'');localStorage.setItem('hidegames.account-name',body.user?.displayName??name);window.dispatchEvent(new Event('hidegames-auth'));setLoggedIn(true);setStatus(`${body.user?.displayName??name} としてログインしました`);setPassword('')}catch(error){setStatus(error instanceof Error?error.message:'認証に失敗しました')}}
  const logout=()=>{localStorage.removeItem('hidegames.auth-token');localStorage.removeItem('hidegames.account-name');window.dispatchEvent(new Event('hidegames-auth'));setLoggedIn(false);setStatus('ログアウトしました')}
...
function SettingsScreenV2({brightness,setBrightness,shortcut,setShortcut}:{brightness:number;setBrightness:(v:number)=>void;shortcut:string;setShortcut:(value:string)=>void}){
  type Preferences={bright:boolean;chat:boolean;notifications:boolean;fontScale:number;colorSafe:boolean;bgm:number;sfx:number;voice:number;youtube:number}
  const [prefs,setPrefs]=useState<Preferences>(()=>{try{return{bright:true,chat:true,notifications:true,fontScale:100,colorSafe:false,bgm:70,sfx:70,voice:80,youtube:80,...JSON.parse(localStorage.getItem('hidegames.preferences')??'{}')}}catch{return{bright:true,chat:true,notifications:true,fontScale:100,colorSafe:false,bgm:70,sfx:70,voice:80,youtube:80}}})
  const [status,setStatus]=useState('')
  useEffect(()=>{localStorage.setItem('hidegames.preferences',JSON.stringify(prefs));document.documentElement.style.fontSize=`${prefs.fontScale}%`;document.documentElement.dataset.colorSafe=`${prefs.colorSafe}`;window.dispatchEvent(new Event('hidegames-preferences'))},[prefs])
  ...
  return <div className="page settings-page"><header className="page-title"><div><p className="eyebrow">PREFERENCES</p><h1>設定</h1><p>離席キー、明るさ、音量、通知、見やすさを自分好みに。</p></div></header><section className="settings-group"><div><span className="setting-icon"><MonitorDown/></span><h2>離席モード</h2><p>キーを押すと、ゲームを全員で停止してHideGamesを隠します。</p></div><div className="setting-fields"><label>離席・復帰キー<input value={shortcut} onChange={event=>setShortcut(event.target.value)}/></label><button className="secondary" onClick={saveShortcut}>キー競合を確認して保存</button>{status&&<p className="setting-status">{status}</p>}</div></section><section className="settings-group"><div><span className="setting-icon"><Sparkles/></span><h2>ディスプレイの明るさ</h2><p>離席時に明るさを上げ、復帰時に元へ戻します。</p></div><div className="setting-fields"><label className="switch-line"><span>離席時に明るくする</span><button className={`switch ${prefs.bright?'on':''}`} onClick={()=>update('bright',!prefs.bright)} aria-label="明るさ連動を切り替える"><i/></button></label>{prefs.bright&&<label>離席時の明るさ <output>{brightness}%</output><input type="range" min="40" max="100" value={brightness} onChange={event=>setBrightness(Number(event.target.value))}/></label>}<button className="text-button" onClick={async()=>{const result=await window.hideGamesDesktop?.setBrightness(brightness);setStatus(result?.supported?'明るさをテストしました':'このディスプレイでは物理的な明るさ変更に対応していません')}}>明るさをテスト</button></div></section><section className="settings-group"><div><span className="setting-icon"><MessageCircle/></span><h2>通知とチャット</h2><p>ゲーム中のチャットドックと新着通知を設定します。</p></div><div className="setting-fields"><label className="switch-line"><span>ゲーム中にチャットを表示</span><button className={`switch ${prefs.chat?'on':''}`} onClick={()=>update('chat',!prefs.chat)}><i/></button></label><label className="switch-line"><span>招待・チャットを通知</span><button className={`switch ${prefs.notifications?'on':''}`} onClick={()=>update('notifications',!prefs.notifications)}><i/></button></label></div></section><section className="settings-group"><div><span className="setting-icon"><Radio/></span><h2>音量</h2><p>各種類の音量を個別に保存します。対応する再生機能に反映されます。</p></div><div className="setting-fields">{([{key:'bgm',label:'BGM'},{key:'sfx',label:'効果音'},{key:'voice',label:'ボイスチャット'},{key:'youtube',label:'YouTube'}] as const).map(item=><label key={item.key}>{item.label} <output>{prefs[item.key]}%</output><input type="range" min="0" max="100" value={prefs[item.key]} onChange={event=>update(item.key,Number(event.target.value))}/></label>)}</div></section><section className="settings-group"><div><span className="setting-icon"><CircleHelp/></span><h2>アクセシビリティ</h2><p>文字の大きさと色覚配慮テーマを変更できます。</p></div><div className="setting-fields"><label>文字サイズ <output>{prefs.fontScale}%</output><input type="range" min="90" max="125" value={prefs.fontScale} onChange={event=>update('fontScale',Number(event.target.value))}/></label><label className="switch-line"><span>色覚配慮テーマ</span><button className={`switch ${prefs.colorSafe?'on':''}`} onClick={()=>update('colorSafe',!prefs.colorSafe)}><i/></button></label></div></section></div>
}
