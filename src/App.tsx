...
function ShogiGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  const legalDrop=(kind:ShogiBase,row:number,col:number)=>{if(board[row][col])return false;if((kind==='P'||kind==='L')&&(turn==='b'?row===0:row===8))return false;if(kind==='N'&&(turn==='b'?row<=1:row>=7))return false;if(kind==='P'&&board.some(line=>line[col]===`${turn}P`))return false;return true}
  const drop=(row:number,col:number)=>{if(paused||winner||turn!==myColor||!selectedHand||!legalDrop(selectedHand,row,col))return;const next=board.map(line=>[...line]);next[row][col]=`${turn}${selectedHand}` as ShogiPiece;const nextHands={...hands,[turn]:hands[turn].filter((kind,index)=>kind!==selectedHand||index!==hands[turn].indexOf(selectedHand))};const nextTurn=turn==='b'?'w':'b';setBoard(next);setHands(nextHands);setTurn(nextTurn);setSelectedHand(null);syncState({board:next,hands:nextHands,turn:nextTurn,winner:null})}
  const move=(row:number,col:number)=>{if(paused||winner||turn!==myColor)return;if(selectedHand)return drop(row,col);const piece=board[row][col];if(!selected){if(piece?.[0]===turn)setSelected([row,col]);return}if(selected[0]===row&&selected[1]===col){setSelected(null);return}const moving=board[selected[0]][selected[1]];if(piece?.[0]===turn){setSelected([row,col]);return}if(!valid(selected,[row,col],moving))return;const next=board.map(line=>[...line]);const captured=next[row][col];const resolved=promoted(moving!,selected[0],row);next[row][col]=resolved;next[selected[0]][selected[1]]=null;const nextHands=captured&&captured[1]!=='K'?{...hands,[turn]:[...hands[turn],base(captured)]}:hands;const won=captured?.[1]==='K'?turn:null;const nextTurn=turn==='b'?'w':'b';setBoard(next);setHands(nextHands);setTurn(nextTurn);setWinner(won);setSelected(null);syncState({board:next,hands:nextHands,turn:nextTurn,winner:won})}
  ...
  return <div className="board-game shogi-game"><div className="game-top"><div><span className="tag">BOARD / SHOGI</span><h1>将棋</h1></div><div className="turn-pill">{winner?`${winner==='b'?'先手':'後手'}の勝ち`:`${turn===myColor?'あなた':'相手'}の番`}</div></div><p className="game-hint">持ち駒は盤上の空きマスへ打てます。成りを選択でき、二歩や行き所のない駒打ちは防止されます。</p><section className="go-actions"><span>あなたの持ち駒</span>{hands[myColor??'b'].length?hands[myColor??'b'].map((kind,index)=><button key={`${kind}-${index}`} className={`secondary ${selectedHand===kind?'selected':''}`} disabled={paused||Boolean(winner)||turn!==myColor} onClick={()=>{setSelected(null);setSelectedHand(selectedHand===kind?null:kind)}}>{glyph[`${turn}${kind}` as ShogiPiece]}</button>):<small>なし</small>}</section><div className="shogi-board">{board.map((line,row)=>line.map((piece,col)=>{const active=selected?.[0]===row&&selected?.[1]===col;const destination=selected&&valid(selected,[row,col]);return <button key={`${row}-${col}`} disabled={paused||Boolean(winner)||turn!==myColor} className={`${active?'active':''} ${destination?'move-target':''}`} onClick={()=>move(row,col)}>{piece&&<span className={piece[0]==='w'?'opponent-piece':''}>{glyph[piece]}</span>}</button>}))}</div>{winner&&<button className="secondary center" disabled={!host} onClick={reset}>{host?'新しい対局':'ホストが再戦を開始します'}</button>}</div>
}
...
function GoGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  useEffect(() => { const state = sharedState as { board?: Stone[][]; turn?: 'b'|'w'; captures?: {b:number;w:number}; passes?: number; winner?: 'b'|'w'|null; previousBoard?: Stone[][] | null } | undefined; if (state?.board?.length === size && state.turn) { setBoard(state.board);setTurn(state.turn);setCaptures(state.captures ?? {b:0,w:0});setPasses(state.passes ?? 0);setWinner(state.winner ?? null);setPreviousBoard(state.previousBoard?.length===size?state.previousBoard:null);setRuleNotice('') } }, [sharedState])
  ...
  const place = (row: number, col: number) => {
    ...
    const nextCaptures={...captures,[turn]:captures[turn]+taken};const nextTurn=other;setBoard(next);setTurn(nextTurn);setCaptures(nextCaptures);setPasses(0);setPreviousBoard(board);setRuleNotice('');syncState({board:next,turn:nextTurn,captures:nextCaptures,passes:0,winner:null,previousBoard:board})
  }
  const playerIndex=members.findIndex(member=>member.id===playerId); const myColor=playerIndex===0?'b':playerIndex===1?'w':null; const host=members[0]?.id===playerId
  const pass = () => { if(paused||winner||turn!==myColor)return;const nextPasses=passes+1;const stoneScore={b:board.flat().filter(stone=>stone==='b').length+captures.b,w:board.flat().filter(stone=>stone==='w').length+captures.w};const won=nextPasses>=2?(stoneScore.b===stoneScore.w?null:stoneScore.b>stoneScore.w?'b':'w'):null;const nextTurn=turn==='b'?'w':'b';setTurn(nextTurn);setPasses(nextPasses);setWinner(won);setPreviousBoard(null);setRuleNotice('');syncState({board,turn:nextTurn,captures,passes:nextPasses,winner:won,previousBoard:null}) }
  const reset=()=>{if(!host)return;const next=blank();setBoard(next);setTurn('b');setCaptures({b:0,w:0});setPasses(0);setWinner(null);setPreviousBoard(null);setRuleNotice('');syncState({board:next,turn:'b',captures:{b:0,w:0},passes:0,winner:null,previousBoard:null})}
...
function MahjongGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  type State = { wall?: Tile[]; hands?: Record<string, Tile[]>; turn?: string; discards?: Tile[]; winner?: string; winType?: 'tsumo'|'ron'; riichi?: Record<string,boolean>; lastDiscard?: { tile:Tile; owner:string }; exhausted?: boolean; draw?: boolean }
  ...
  const tenpai=(hand:Tile[])=>{const choices:Tile[]=['m','p','s'].flatMap(suit=>Array.from({length:9},(_,rank)=>({suit:suit as Tile['suit'],rank:rank+1}))).concat(Array.from({length:7},(_,rank)=>({suit:'z' as const,rank:rank+1})));return choices.some(tile=>winningHand([...hand,tile]))}
  const start = () => { if(!host)return;const wall = buildWall(); const nextHands = Object.fromEntries(members.map((member, index) => [member.id, wall.slice(index * 13, index * 13 + 13)])); setDeclaringRiichi(false);syncState({ wall: wall.slice(members.length * 13), hands: nextHands, turn: members[0]?.id, discards: [], riichi: {}, lastDiscard: undefined, draw: false }) }
  const discard = (index: number) => { if (paused || active !== playerId || state.winner || state.draw || !state.wall?.length) return; const hand = mine.length === 13 ? [...mine, state.wall[0]] : mine; const discarded = hand[index]; if (!discarded) return; const nextHand=hand.filter((_, tileIndex) => tileIndex !== index); if(declaringRiichi&&!tenpai(nextHand))return;const nextHands = { ...hands, [playerId]: nextHand }; const nextTurn = members[(members.findIndex(member => member.id === playerId) + 1) % members.length]?.id ?? playerId; const remaining = state.wall.slice(1); setDeclaringRiichi(false);syncState({ ...state, wall: remaining, hands: nextHands, discards: [...(state.discards ?? []), discarded], turn: nextTurn, riichi: declaringRiichi?{...(state.riichi??{}),[playerId]:true}:state.riichi, lastDiscard:{tile:discarded,owner:playerId}, exhausted:remaining.length===0 }) }
  const tsumo=()=>{const hand=mine.length===13&&state.wall?.[0]?[...mine,state.wall[0]]:mine;if(paused||active!==playerId||!winningHand(hand))return;syncState({...state,winner:playerId,winType:'tsumo',lastDiscard:undefined})}
  const ron=()=>{if(paused||state.winner||state.draw||!state.lastDiscard||state.lastDiscard.owner===playerId||!winningHand([...mine,state.lastDiscard.tile]))return;syncState({...state,winner:playerId,winType:'ron'})}
  ...
  const canTsumo=active===playerId&&winningHand(visibleHand);const canRon=Boolean(state.lastDiscard&&state.lastDiscard.owner!==playerId&&winningHand([...mine,state.lastDiscard.tile]));
  return <div className="board-game mahjong-game"><div className="game-top"><div><span className="tag">TABLE / RIICHI LITE</span><h1>麻雀</h1></div><div className="turn-pill">{state.winner ? `${members.find(m => m.id === state.winner)?.name} の${state.winType==='ron'?'ロン':'ツモ'}和了` : state.draw?'山切れで流局':state.exhausted?'最後の捨て牌を確認中':active ? `${members.find(m => m.id === active)?.name ?? 'プレイヤー'} の番` : '卓を開始'}</div></div>{!state.wall ? <section className="mahjong-start panel"><h2>4面子1雀頭を作ろう</h2><p>山から1枚引き、不要な牌を1枚捨てます。順子・刻子と雀頭で14枚をそろえると和了です。</p><button className="primary" disabled={paused||!host} onClick={start}>{host?'対局を開始':'ホストが対局を開始します'}</button></section> : <><section className="mahjong-table panel"><div><b>残り {state.wall.length} 枚</b><span>捨て牌: {(state.discards ?? []).slice(-12).map(tileLabel).join(' ') || 'なし'}</span></div><p>{state.lastDiscard&&<>{members.find(member=>member.id===state.lastDiscard?.owner)?.name} が {tileLabel(state.lastDiscard.tile)} を捨てました。 </>}{state.exhausted?'ロンがなければ流局を確定します。':active === playerId ? '牌を1枚選んで捨ててください。' : '相手の手番を待っています。'}</p><div className="go-actions">{canTsumo&&<button className="primary" disabled={paused||Boolean(state.winner)||state.draw} onClick={tsumo}>ツモ</button>}{canRon&&<button className="primary" disabled={paused||Boolean(state.winner)||state.draw} onClick={ron}>ロン</button>}{state.exhausted&&active===playerId&&<button className="secondary" disabled={paused||Boolean(state.winner)||state.draw} onClick={draw}>流局を確定</button>}{active===playerId&&!state.exhausted&&mine.length===13&&!state.riichi?.[playerId]&&<button className={`secondary ${declaringRiichi?'selected':''}`} disabled={paused||Boolean(state.winner)||state.draw} onClick={()=>setDeclaringRiichi(!declaringRiichi)}>{declaringRiichi?'リーチ牌を選択中':'リーチを宣言する'}</button>}</div>{declaringRiichi&&<small>テンパイになる牌を捨ててください。</small>}</section><section className="mahjong-hand">{visibleHand.map((tile, index) => <button key={`${tileKey(tile)}-${index}`} className={`mahjong-tile ${tile.suit}`} disabled={paused || active !== playerId || Boolean(state.winner)||state.draw||Boolean(state.exhausted)} onClick={() => discard(index)}>{tileLabel(tile)}</button>)}</section></>}</div>
}
