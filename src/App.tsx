...
function PuzzleGame({ paused, sharedState, syncState, members, playerId }: { paused: boolean; sharedState: unknown; syncState: (state: unknown) => void; members: RoomMember[]; playerId: string }) {
  ...
  const drop = () => { if (paused || turn !== playerId || state.finished) return; let landed = active; while (canPlace({ ...landed, y: landed.y + 1 })) landed = { ...landed, y: landed.y + 1 }; const settled = board.map(row => [...row]); shapeCells(landed).forEach(cell => { if (cell.y >= 0) settled[cell.y][cell.x] = (cell.value % 4 + 1) as Block }); const resolved=resolvePuzzleChains(settled);const nextTurn=members[(members.findIndex(member=>member.id===turn)+1)%members.length]?.id??playerId;const nextScore={...score,[playerId]:(score[playerId]??0)+resolved.cleared*Math.max(1,resolved.chains)};const nextActive={x:2,y:0,shape:(active.shape+1)%tetrominoes.length,rot:0};syncState({board:resolved.board,active:nextActive,turn:nextTurn,score:nextScore,lastChain:resolved.chains,finished:!canPlace(nextActive)}) }
  const activeCells = new Map(shapeCells(active).map(cell => [`${cell.x}:${cell.y}`, ((cell.value % 4) + 1) as Block]))
  return <div className="board-game falling-game"><div className="game-top"><div><span className="tag">COLOUR / CHAIN PUZZLE</span><h1>落ちものパズル</h1></div><div className="turn-pill">{state.finished ? 'ゲーム終了' : `${members.find(m => m.id === turn)?.name ?? 'あなた'} の番`}</div></div><section className="falling-layout"><div className="falling-board puzzle-board">{board.flatMap((row,y)=>row.map((value,x)=><i key={`${x}-${y}`} className={`block orb c${activeCells.get(`${x}:${y}`) ?? value}`}/>))}</div><section className="falling-side panel"><h2>4つつなげて消す</h2><p>同じ色を4つ以上隣接させると消え、落下後も続けて判定されます。</p>{state.lastChain&&<p className="setting-status">{state.lastChain} 連鎖</p>}<div className="falling-controls"><button disabled={paused || turn !== playerId} onClick={()=>move(-1)}>左</button><button disabled={paused || turn !== playerId} onClick={()=>move(0,1)}>回転</button><button disabled={paused || turn !== playerId} onClick={()=>move(1)}>右</button><button className="primary" disabled={paused || turn !== playerId} onClick={drop}>落とす</button></div><div className="falling-score">{members.map(member=><div key={member.id}><span>{member.name}</span><b>{score[member.id]??0} POINT</b></div>)}</div></section></section></div>
}
